...
def bt_build(src_dir, build_dir):
    ...
    with new_wd(build_dir):
        subprocess.run(f"{configure_path} --prefix {build_dir} --disable-debug-info --enable-python-bindings --disable-man-pages", shell=True, check=True)
...
def bt_patch_lib(path):
    if sys.platform == "darwin":
        ...
        bt2_path = next((x.strip().rpartition(" (compatibility version")[0] for x in lines if "libbabeltrace2.0.dylib" in x), None)
        if bt2_path is not None:
            subprocess.run(f"install_name_tool -change {bt2_path} @loader_path/libbabeltrace2.0.dylib {path}", shell=True, check=True)
    else:
        subprocess.run(f"patchelf --set-rpath '$ORIGIN' {path}", shell=True, check=True)
        subprocess.run(f"patchelf --replace-needed libbabeltrace2.so.0 libbabeltrace2.so {path}", shell=True, check=True)


def bt_get_plugin_paths(build_dir):
    paths: list[Path] = []
    for plugin in ["ctf", "text", "utils"]:
        paths.append(build_dir / "./src/plugins" / plugin / ".libs" / f"babeltrace-plugin-{plugin}.so")
    return paths
...
class build(_build):
    def run(self):
        ...
        for path in [bt_get_babeltrace2_path(build_dir), *bt_get_plugin_paths(build_dir), *bt_get_bindings_paths(build_dir)]:
